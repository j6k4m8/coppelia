...
-   [x] UI thrash: scroll gets janky again during updates (investigate repaint storms)
-   [x] Android: generate app icons from `assets/logo_app_icon.svg`.
...
-   [ ] gestures:
    -   [x] on mobile now playing bottom panel or side panel, swipe up to open full now playing screen, swipe left/right on the song art and text to skip tracks. should "smooth pursuit" the finger, with next/prev track art and text peeking out on sides when swiping.
    -   [x] on mobile Now Playing popover screen, swipe down to dismiss, swipe left and right to skip tracks (should have album art of next/prev track peek out on sides when swiping)
    -   [x] on mobile homepage, swipe right on "background" or left edge to open/close side nav. and swipe left on side nav or the darkened background to close side nav.
