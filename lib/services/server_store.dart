...
class ServerStore {
  ...
  // Keep removals, address edits, and active selection in invocation order.
  ...
  Future<ServerBootstrapResult> bootstrap() => _serialized(() async {
        ...
        if (pendingRemovals.isNotEmpty) {
          ...
          servers = servers
              .where((server) => !removedIds.contains(server.id))
              .toList();
          sessions = {
        ...
        if (legacy != null) {
          ...
          await preferences.setString(
              _pendingLegacyDataMigrationKey, server.id);
        ...
        return ServerBootstrapResult(
          ...
          active: active,
          migratedServerId:
              preferences.getString(_pendingLegacyDataMigrationKey),
        );
      });
  ...
  Future<StoredServerSession> addAuthenticatedServer(
    ...
    String? name,
      ...
      _serialized(() async {
        ...
        final normalizedUrl =
            JellyfinClient.normalizeServerUrl(session.serverUrl);
        ...
        if (existing != null) {
          ...
          final activeServer =
              existingServer.copyWith(activeAddressId: address.id);
          await _saveServers(
            preferences,
            servers
                .map(
  ...
  Future<List<SavedServer>> renameServer(String serverId, String name) =>
      _serialized(() async {
        final preferences = await SharedPreferences.getInstance();
        final servers = await _loadServers(preferences);
  ...
  Future<List<SavedServer>> addAddress(
    ...
    required String url,
      ...
      _serialized(() async {
        ...
        servers[index] =
            server.copyWith(addresses: [...server.addresses, address]);
        await _saveServers(preferences, servers);
        return servers;
      });
  ...
  Future<List<SavedServer>> removeAddress(String serverId, String addressId) =>
      _serialized(() async {
        final preferences = await SharedPreferences.getInstance();
        final servers = await _loadServers(preferences);
        final index = servers.indexWhere((server) => server.id == serverId);
  ...
  Future<StoredServerSession?> removeServer(String serverId) =>
      _serialized(() async {
        final preferences = await SharedPreferences.getInstance();
        final servers = await _loadServers(preferences);
        final sessions = _loadSessions(preferences);
